...
This workflow applies to Cursor, Codex, and every other coding agent used on MFGFlow. StockSignal remains the inventory module. The human repository owner is Valence Mwigani. Agents implement and verify changes, but commits must appear as ordinary commits authored only by Valence.
