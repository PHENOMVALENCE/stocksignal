-- Brand future low-stock notifications as JengaFlow without rewriting history.
...
begin
  ...
  set quantity = next_quantity, alert_active = next_alert
  where id = item.id;

  insert into public.stock_movements (
    inventory_item_id, type, quantity, previous_quantity, new_quantity, notes
  )
  values (
    item.id, p_type, p_quantity, item.quantity, next_quantity,
  ...
  if should_alert and item.manager_phone is not null and btrim(item.manager_phone) <> '' then
    ...
    insert into public.notifications (
      inventory_item_id, type, recipient, message, provider, status
    )
    values (
      item.id,
      'LOW_STOCK',
      btrim(item.manager_phone),
      'JENGAFLOW ALERT' || chr(10) || chr(10)
